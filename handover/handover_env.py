...
class HandoverEnv(easysim.SimulatorEnv):
  ...
  def _release_check(self):
        ...
        if len(contact_link) == 0:
          ...
          if np.any(np.isnan(contact_link['position_b_link']['x'])):
            ...
            position = np.ascontiguousarray(
                contact_link['position_b_world']).view(np.float32).reshape(
                    -1, 3)
            position = t3d.transform_points(position)
          else:
            position = np.ascontiguousarray(
                contact_link['position_b_link']).view(np.float32).reshape(
                    -1, 3)

          is_in_release_region = (
              (position[:, 0] > self.cfg.ENV.RELEASE_FINGER_CONTACT_X_RANGE[0])
              & (position[:, 0]
                 < self.cfg.ENV.RELEASE_FINGER_CONTACT_X_RANGE[1]) &
              (position[:, 1] > self.cfg.ENV.RELEASE_FINGER_CONTACT_Y_RANGE[0])
              & (position[:, 1]
                 < self.cfg.ENV.RELEASE_FINGER_CONTACT_Y_RANGE[1]) &
              (position[:, 2]
               > self.cfg.ENV.RELEASE_FINGER_CONTACT_Z_RANGE[0]) &
    ...
    return self._release_step_counter_passive >= self._release_step_thresh or self._release_step_counter_active >= self._release_step_thresh
