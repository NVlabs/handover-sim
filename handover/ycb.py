  ...
  def reset(self, scene_id):
    ...
    if self._bodies == {}:
      for i in self._ycb_ids:
        ...
        body.urdf_file = os.path.join(os.path.dirname(__file__), "data",
        ...
        dof_max_force = self._cfg.ENV.YCB_TRANSLATION_MAX_FORCE + self._cfg.ENV.YCB_ROTATION_MAX_FORCE
        if i == self._ycb_ids[self._ycb_grasp_ind]:
          body.dof_max_force = [dof_max_force] * self._cfg.SIM.NUM_ENVS
        else:
          body.dof_max_force = dof_max_force

        body.dof_position_gain = self._cfg.ENV.YCB_TRANSLATION_POSITION_GAIN + self._cfg.ENV.YCB_ROTATION_POSITION_GAIN
        body.dof_velocity_gain = self._cfg.ENV.YCB_TRANSLATION_VELOCITY_GAIN + self._cfg.ENV.YCB_ROTATION_VELOCITY_GAIN
      ...
      link_collision_filter = [
          self._cfg.ENV.COLLISION_FILTER_YCB[[*self._CLASSES].index(
              self._ycb_ids[self._ycb_grasp_ind])]
      ] * 7
      self.grasped_body.link_collision_filter = [link_collision_filter
                                                ] * self._cfg.SIM.NUM_ENVS
