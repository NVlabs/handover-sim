...
class MANO:
  ...
  def make(self):
    if self._body is None:
      self._model = HandModel45(left_hand=self._mano_side == 'left',
      ...
      body.urdf_file = os.path.join(
          os.path.dirname(__file__), "data", "assets",
          "{}_{}".format(self._subject, self._mano_side), "mano.urdf")
      ...
      body.dof_max_force = self._cfg.ENV.MANO_TRANSLATION_MAX_FORCE + self._cfg.ENV.MANO_ROTATION_MAX_FORCE + self._cfg.ENV.MANO_JOINT_MAX_FORCE
      body.dof_position_gain = self._cfg.ENV.MANO_TRANSLATION_POSITION_GAIN + self._cfg.ENV.MANO_ROTATION_POSITION_GAIN + self._cfg.ENV.MANO_JOINT_POSITION_GAIN
      body.dof_velocity_gain = self._cfg.ENV.MANO_TRANSLATION_VELOCITY_GAIN + self._cfg.ENV.MANO_ROTATION_VELOCITY_GAIN + self._cfg.ENV.MANO_JOINT_VELOCITY_GAIN
    ...
    self._reset_from_mano(self._t[self._frame], self._q[self._frame],
                          self._base_euler[self._frame])
  ...
  def _reset_from_mano(self, trans, mano_pose, base_euler):
    ...
    self.body.initial_dof_position = trans.tolist() + base_euler.tolist(
    ) + angles

  def _set_target_from_mano(self, trans, mano_pose, base_euler):
    angles, basis = self._model.mano_to_angles(mano_pose)
    trans = trans + self._origin - basis @ self._origin
    self.body.dof_target_position = trans.tolist() + base_euler.tolist(
    ) + angles
  ...
  def step(self):
    ...
    if self._frame > self._sid and self._frame <= self._eid:
      self._set_target_from_mano(self._t[self._frame], self._q[self._frame],
                                 self._base_euler[self._frame])
