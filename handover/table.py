...
class Table:
  ...
  def add(self):
    body = easysim.Body()
    body.name = 'table'
    body.urdf_file = os.path.join(os.path.dirname(__file__), "data", "assets",
                                  "table", "table.urdf")
    body.initial_base_position = self._cfg.ENV.TABLE_BASE_POSITION + self._cfg.ENV.TABLE_BASE_ORIENTATION
